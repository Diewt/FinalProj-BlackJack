...
class Dealer(object):
    def __init__(self):
        # List variable to keep track of what is in the dealer's hand
        self.Hand = []
        # Bool Variable to track whether or not an ace is in the hand
        self.Ace = False
        # A variable to track the amount of aces in the dealer's hand
        self.AceAmount = 0
        # A variable to track the value of the dealer's hand, not accounting the ace
        self.Value = 0

    # A function to print out both the dealer's hand and its value
    def printHand(self):
        print("Dealer's Hand: ")
        for i in range(len(self.Hand)):
            print(self.Hand[i])

        print("The Value of the Dealer's Hand: " + str(self.getValue()))

    # A function to add a card to the dealer's hand and record it's value
    ...
    # A function to check whether or not the dealer busted by getting a hand value greater than 21
    def checkBust(self):
        if self.getValue() < 21:
            return False
        else:
    ...
    # A function to check whether or not the dealer should hit or stand according to the rules of blackjack
    def hitOrStand(self):
        if self.getValue() < 17:
            return True
        else:
